...
nano_models.forEach(function(model) {
  test('[Nano ' + model.letter + '] Swap V2 Proxy02 Dodo Swap V2 Token To Token', zemu(model, async (sim, eth) => {
  ...
  const {data} = await contract.populateTransaction.dodoSwapV2TokenToToken(fromToken, toToken, fromTokenAmount, minReturnAmount, dodoPairs, directions, isIncentive, deadLine);
  ...
  await sim.navigateAndCompareSnapshots('.', model.name + '_swap_v2_proxy02_dodo_swap_v2_token_to_token', [right_clicks, 0]);
